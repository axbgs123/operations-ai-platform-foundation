...
- Admin：左侧导航 16/16，可见治理、成员、模型、删除和恢复入口。
- Editor：左侧导航 13/13；无成员/模型/工作区删除入口，后台任务无重试、取消或补偿按钮。
- Viewer：左侧导航 9/9；无栏目、导入及其他写入口；邀请码写按钮不进入 DOM，直接调用成员邀请码 API 返回 403。
- Demo：位于 `/demo`，没有私有 WorkspaceShell 或私有主导航；只读合成数据，可明确进入 `/enter`。
- 跨工作区上下文直接访问返回 404；资源存在性不通过角色错误泄露。
- 16 个模块均从可见主导航到达，路由高亮、面包屑、顶部 Scope 和唯一 `main` landmark 均有 E2E 证据。
...
Darwin 基线共 29 张，位于 `tests/e2e/workbench-visual.spec.ts-snapshots/`：
...
29/29 稳定比较通过。人工逐张检查结论：

- 左侧导航和当前层级清楚；页面保持统一浅色工作台外壳，没有大面积深色旧私有页面。
...
| Web 全量 | `42 files / 163 tests passed` |
| Extension 全量 | `8 files / 39 tests passed` |
| Task 10 路由/移动/视觉 | `3 + 2 + 1` 测试通过；视觉测试包含 29 张基线 |
...
按 `requesting-code-review` 对基线至当前工作树的 tracked、untracked 和 29 张视觉基线清单完成独立只读复核：

- Critical：0
- Important：0
- Ready：Yes

复核未发现跨 workspace/platform 泄漏、权限绕过、开放重定向、真实秘密、Mock 冒充生产结果、正式导航 404 或独立非开发者伪证据。所有新增测试、视觉基线和验收文档必须随本 Task 同一提交固化。
