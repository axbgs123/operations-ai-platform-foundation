...
自动化已证明 0.3.0 的持久设备续签、真实 Popup/Service Worker/content-script 消息边界、Mock Provider 披露、默认关闭遮挡和确定性制品。自动化没有产生 Chrome 所要求的真实用户工具栏或快捷键手势，因此 `captureVisibleTab`、其后的真实整页预览/上传/Web 人工确认仅保持 `not_run`；不得把组件测试写成完整浏览器链通过。
...
| 遮挡默认值 | `passed` | 真实 content overlay 显示“遮挡敏感信息：关”；组件测试也验证整页预览仍默认关闭 |
| 整页滚动与拼接 | `component_tested` | 固定 4000px 合成页、1280×800 viewport 产生 6 次底部稳定性观察；重复的第二张底部截图不进入拼接，保留 5 张切片并拼接为 1280×4000，状态“完整” |
...
5. 实际按一次已显示的整页快捷键；确认预览的“采集 N 屏”中 N 大于 0，完整性与当前页面实际可滚动范围一致，预览尺寸与当前页面一致，并在上传前保持人工确认。固定的“采集 5 屏”和 1280×4000 只属于上方 4000px 合成夹具的组件证据，不是任意真实页面的固定期望值。
