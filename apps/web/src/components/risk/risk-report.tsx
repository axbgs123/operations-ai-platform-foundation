...
const severityStyle = {
  low: "border-sky-800 bg-sky-950/40",
  medium: "border-amber-800 bg-amber-950/40",
  high: "border-rose-800 bg-rose-950/40",
} as const;
  ...
  return (
    <section className="space-y-5 rounded-3xl border border-slate-800 bg-slate-900 p-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm text-slate-400">风险扫描 · {scan.node}</p>
          <h2 className="text-xl font-semibold">可验证风险报告</h2>
        </div>
        {historical || scan.previous_scan_id ? (
          <span className="rounded-full bg-violet-950 px-3 py-1 text-sm text-violet-300">
            历史扫描
          </span>
        ) : null}
      </header>
      ...
      {result?.error_code === "NO_ACTIVE_RISK_EVIDENCE" ? (
        <p className="rounded-xl border border-amber-900 bg-amber-950/40 p-3 text-amber-300">
          未检索到有效规则
        </p>
      ) : null}
        ...
        {result?.findings.map((finding, index) => (
          ...
          >
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full bg-slate-950 px-2 py-1 text-xs">
                {finding.origin === "rag"
                  ? "RAG 辅助判断"
            ...
            {finding.requires_human_review ? (
              <p className="text-sm text-amber-200">需要人工确认或复核</p>
            ) : null}
            <p className="text-sm text-slate-300">原因：{finding.reason}</p>
            <p className="text-sm text-slate-300">
              建议：{finding.suggestion}
            </p>
            {finding.citations.map((citation) => (
              <blockquote
                className="rounded-xl border-l-2 border-cyan-500 bg-slate-950 p-3 text-sm"
      ...
      {result ? (
        <footer className="space-y-2 border-t border-slate-800 pt-4 text-xs text-slate-400">
          <p>
          ...
          <p className="font-medium text-slate-300">{result.disclaimer}</p>
        </footer>
      ) : null}
    </section>
  );
