...
function SnapshotsPanel({
  ...
  if (!detail.snapshots.length) {
    return (
      <EmptyState
        description="添加并人工确认数据快照后，可查看当前数据；至少两条同口径快照才显示趋势。"
        title="还没有数据快照"
      />
    );
  }
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-[var(--text-secondary)]">
                  ...
                  {snapshot.source} · {snapshot.maturity_bucket}
                </p>
              </div>
              ...
              <table className="w-full min-w-[560px] text-left text-sm">
                <thead>
                  <tr className="text-[var(--text-secondary)]">
                    ...
                    <th className="py-2">规范化值</th>
                    <th className="py-2">OCR/识别置信度</th>
                  </tr>
                </thead>
                <tbody>
          ...
          {items.map((item, index) => (
            <li className="rounded-lg bg-slate-50 p-3 text-sm" key={`${title}-${index}`}>
              ...
              <p className="mt-1 text-[var(--text-secondary)]">
                置信度 {item.confidence} · Evidence {item.evidence_ids.join("、")}
              </p>
            </li>
          ))}
        </ul>
...
function AnalysisPanel({
  ...
  if (run.status !== "succeeded" || !run.report) {
    return (
      <ErrorState
        description={`任务状态：${run.status}；安全错误码：${run.error_code ?? "当前记录未提供"}`}
    ...
    <div className="space-y-5">
      <Panel title="数据表现">
        ...
        <p className="mt-2 text-sm text-[var(--text-secondary)]">
          Evidence：{report.data_performance.evidence_ids.join("、")}
        </p>
      </Panel>
      ...
      <Panel title="证据、样本与版本">
        <DefinitionList
          items={[
            ["Evidence 引用", report.evidence.map((item) => item.evidence_id).join("、")],
            ["样本数", run.snapshot_ids.length],
            ["基准范围", run.benchmark_run_id],
            ["置信度", report.confidence],
            ["模型", `${run.model_provider} / ${run.model_version}`],
            ["Prompt 版本", run.prompt_version],
            ["分析版本", run.algorithm_version],
            ["基准算法版本", run.benchmark_algorithm_version],
            ["反馈", "有用 / 无用（由现有分析操作记录）"],
    ...
    <div className="space-y-5">
      <Panel title="联合扫描状态">
        <DefinitionList
          items={[
            ["扫描节点", latest.node],
            ["状态", latest.status],
            ["OCR 状态", latest.result?.ocr_status ?? "当前记录未提供"],
            ["无有效证据", latest.result?.error_code === "NO_ACTIVE_RISK_EVIDENCE"
              ? "未检索到有效规则"
              : "否"],
            ["规则版本", latest.rule_version],
            ["证据版本", latest.evidence_version],
            ["Embedding", `${latest.embedding_model_id} / ${latest.embedding_version}`],
            ["RAG 模型", latest.rag_model_version],
          ]}
        />
      </Panel>
      {inProgress ? (
        <Panel
          ...
          title="扫描正在进行"
        >
          <p role="status">当前状态：{latest.status}，请稍后刷新。</p>
        </Panel>
      ) : null}
      {failed ? (
        <ErrorState
      ...
      {assessment ? (
        <Panel title={
          ...
          <ul className="space-y-3">
            {findings.map((finding, index) => (
              <li className="rounded-lg border p-4" key={`${finding.risk_type}-${index}`}>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span>{finding.origin === "deterministic"
                    ? "确定性规则命中"
                    : finding.origin === "rag"
                      ? "RAG 辅助判断"
                      : "确定性规则 + RAG"}</span>
                </div>
                <p className="mt-2 text-sm">位置：{finding.region}；命中：{finding.matched_content}</p>
                {finding.ocr_confidence !== null ? (
                  <p className="mt-1 text-sm">
                    OCR 置信度：{Math.round((finding.ocr_confidence ?? 0) * 100)}%
                    {finding.requires_human_review ? "（低置信度，要求人工复核）" : ""}
                  </p>
                ) : null}
            ...
            <p>RAG 未检索到有效规则；仅保留确定性规则结果，不得视为安全通过。</p>
          ) : (
      ...
      <Panel title="历史扫描与复检">
        <ol className="space-y-2 text-sm">
          {scans.map((item) => (
            <li key={item.id}>
              {new Date(item.created_at).toLocaleString("zh-CN")} · {item.status}
  ...
  return (
    <div className="space-y-4">
      {detail.generation_records.map((record) => (
        <Panel
          description={`${record.provider} / ${record.model_id}`}
          key={record.id}
          title={`封面生成任务 · ${record.status}`}
        >
          <DefinitionList
            items={[
              ["Provider 与模型安全状态", `${record.provider} / ${record.model_id}`],
              ["生成合同版本", record.contract_version],
              ["账号风格版本", record.account_style_version ?? "当前记录未提供"],
              ["栏目覆盖版本", record.column_override_version ?? "当前记录未提供"],
              ["事实来源与确认状态", record.confirmed_facts_version ?? "当前记录未提供"],
              ["爆款引用", record.viral_reference_count ?? "当前记录未提供"],
              ["生成预设版本", record.preset_version ?? "当前记录未提供"],
              ["原始生成结果", record.original_result ?? "当前记录未提供"],
              ["最终采用结果", record.final_result ?? "当前记录未提供"],
              ["采用状态", record.adoption_status ?? "当前记录未提供"],
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <DetailTabs
        ...
        tabs={[
          ...
          { id: "snapshots", label: "数据快照", panel: <SnapshotsPanel detail={detail} /> },
          {
            id: "analysis",
            label: "分析",
            panel: (
...
export function ContentDetailPage({
  ...
  if (!context || state.status === "loading") {
    return <Skeleton label="正在加载内容详情" />;
  }
