...
export const PAGE_GUIDANCE_CATALOG: Record<OperatorPageId, PageGuidanceEntry> = {
  ...
  settingsModels: guidance(
    action("write", "配置模型、预算并执行受控验收"), action("contact", "联系管理员配置模型和费用上限"), action("contact", "联系管理员配置模型和费用上限"),
    ["选择固定模型能力和服务地域。", "输入密钥并设置并发、次数和每日费用上限。", "只有明确授权后才进行一次受控真实验收。"],
    "没有每日费用上限或明确授权时，系统不会发起真实模型调用。",
  ),
};
