...
export const PAGE_GUIDANCE_CATALOG: Record<OperatorPageId, PageGuidanceEntry> = {
  ...
  hotspots: guidance(
    action("write", "选择已确认热点和同平台账号开始核实"),
    action("write", "选择已确认热点和同平台账号开始核实"),
    action("read", "查看已有研究和创作候选"),
    [
      "先用浏览器扩展截取公开热点榜，并人工确认识别出的热点。",
      "选择同平台账号，让已验证的文字模型联网核实公开信息。",
      "检查来源、标题和文案候选，保存后继续核对事实和发布风险。",
    ],
    "没有已确认热点、同平台账号或已验证的联网模型时，系统不会生成结果。",
    steps(
      "已确认热点：人工检查过的截图识别结果。",
      "联网核实：模型通过自身的联网能力查找公开来源。",
      "带来源草稿：每个候选必须引用本次研究返回的网址。",
    ),
  ),
  generation: guidance(
